...
return notStartedYet();};this.setObjective=function(data){if(isStarted){var p1='cmi.objectives.',n=scorm.getObjectiveByID(data.id),i=0,result='false',f=false;scorm.debug(settings.prefix+": Setting Objective at "+n+" (This may be false)");if(isBadValue(n)){n=scorm.getvalue(p1+'_count');if(n==='false'){scorm.debug(settings.prefix+": LMS is return false, can not proceed, check error codes");return n;}
...
if(parseFloat(progressMeasure,10)>=parseFloat(settings.completion_threshold,10)){scorm.setvalue('cmi.completion_status','completed');}else{scorm.setvalue('cmi.completion_status','incomplete');}
if(parseFloat(scoreScaled,10)>=parseFloat(settings.scaled_passing_score,10)){scorm.setvalue('cmi.success_status','passed');}else{scorm.setvalue('cmi.success_status','failed');}};this.happyEnding=function(){if(isStarted){scorm.setvalue('cmi.score.scaled','1');scorm.setvalue('cmi.score.raw','1');scorm.setvalue('cmi.success_status','passed');return scorm.setvalue('cmi.completion_status','completed');}
