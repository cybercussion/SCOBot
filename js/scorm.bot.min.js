...
return true;}
function centisecsToISODuration(n,bPrecise){var str="P",nCs=Math.max(n,0),nY=0,nM=0,nD=0,nH=0,nMin=0,nS=0;nCs=Math.round(nCs);if(bPrecise===true){nD=Math.floor(nCs/8640000);}else{nY=Math.floor(nCs/3155760000);nCs-=nY*3155760000;nM=Math.floor(nCs/262980000);nCs-=nM*262980000;nD=Math.floor(nCs/8640000);}
...
function centisecsToSCORM12Duration(n){var bTruncated=false,str,nH,nCs,nM,nS;n=Math.round(n);nH=Math.floor(n/360000);nCs=n-nH*360000;nM=Math.floor(nCs/6000);nCs=nCs-nM*6000;nS=Math.floor(nCs/100);nCs=nCs-nS*100;if(nH>9999){nH=9999;bTruncated=true;}
...
debug(settings.prefix+": "+n+" Get Aborted, connection not initialized! "+API.isActive,2);return'false';};this.setvalue=function(n,v){var s=false,lms=API.path,ec=0,nn=null,ig=false;$(self).triggerHandler({'type':"setvalue",'n':n,'v':v});if(API.isActive){switch(API.version){case"1.2":if(lms.LMSGetValue('cmi.core.lesson_mode')==="normal"){switch(n){case"cmi.location":if(v.length>255){debug(settings.prefix+": Warning, your bookmark is over the limit!!",2);}
...
break;case"2004":if(lms.GetValue('cmi.mode')==="normal"){switch(n){case"cmi.location":if(v.length>1000){debug(settings.prefix+": Warning, your bookmark is over the limit!!",2);}
...
debug(settings.prefix+": "+n+" Set Aborted, connection not initialized! Locate where you called it after you Terminated.",2);return'false';};this.commit=function(){var s=false,lms=API.path,ec=0,session_secs=0,saveDate=new Date();session_secs=(saveDate.getTime()-settings.startDate.getTime())/1000;if(API.isActive){debug(settings.prefix+": Committing data",3);switch(API.version){case"1.2":self.setvalue("cmi.core.session_time",centisecsToSCORM12Duration(session_secs*100));s=lms.LMSCommit("");break;case"2004":self.setvalue("cmi.session_time",centisecsToISODuration(session_secs*100,true));s=lms.Commit("");break;default:break;}
...
function checkProgress(){if(isStarted){var response={},scoreRaw=0,tmpRaw=0,scoreMax=0,scoreMin=0,scoreScaled=1,progressMeasure=0,completionStatus='',totalObjectivesCompleted=0,totalKnownObjectives=parseInt(scorm.getvalue('cmi.objectives._count'),10),totalKnownInteractions=parseInt(scorm.getvalue('cmi.interactions._count'),10),i=0,count=0;if(settings.totalInteractions===0||settings.totalObjectives===0){scorm.debug(settings.prefix+": Sorry, I cannot calculate Progress as the totalInteractions and or Objectives are zero",2);return'false';}
...
progressMeasure=(totalObjectivesCompleted/settings.totalObjectives).toString();scorm.setvalue('cmi.progress_measure',trueRound(progressMeasure,7));if(parseFloat(progressMeasure,10)>=parseFloat(settings.completion_threshold,10)){scorm.setvalue('cmi.completion_status','completed');}else{scorm.setvalue('cmi.completion_status','incomplete');}
if(parseFloat(scoreScaled,10)>=parseFloat(settings.scaled_passing_score,10)){scorm.setvalue('cmi.success_status','passed');}else{scorm.setvalue('cmi.success_status','failed');}
...
function getCommentsFromLMS(){if(isStarted){var p1="cmi.comments_from_lms.",count=scorm.getvalue(p1+'_count'),response=[],obj={},i=0;if(!isBadValue(count)){return'false';}
...
return'false';}
return notStartedYet();};this.getSecondsFromStart=function(){return settings.startTime-currentTime();};this.setInteraction=function(data){if(isStarted){var n,m,i,j,p,p1='cmi.interactions.',p2='',orig_timestamp=data.timestamp||scorm.isoStringToDate(scorm.getvalue(p1+scorm.getInteractionByID(data.id)+'.timestamp')),timestamp,latency,namespace,result;if(!$.isPlainObject(data)){scorm.debug(settings.prefix+": Developer, your not passing a {object} argument!!  Got "+typeof(data)+" instead.",1);return'false';}
...
return result;}
return notStartedYet();};this.getInteraction=function(id){if(isStarted){var n='false',p1='cmi.interactions.',m=0,p=0,i=0,obj={};n=scorm.getInteractionByID(id);if(n==='false'){return n;}
...
return notStartedYet();};this.setObjective=function(data){if(isStarted){var p1='cmi.objectives.',n=scorm.getObjectiveByID(data.id),i=0,result='false';scorm.debug(settings.prefix+": Setting Objective at "+n+" (This may be false)");if(isBadValue(n)){n=scorm.getvalue(p1+'_count');scorm.debug(settings.prefix+": Objective "+data.id+" was not found.  Adding new at "+n+" "+data.description);p1+=n+".";if($.isPlainObject(data.score)){result=scorm.setvalue(p1+'id',data.id.toString());result=scorm.setvalue(p1+'score.scaled',trueRound(data.score.scaled,7).toString());result=scorm.setvalue(p1+'score.raw',trueRound(data.score.raw,7).toString());result=scorm.setvalue(p1+'score.min',trueRound(data.score.min,7).toString());result=scorm.setvalue(p1+'score.max',trueRound(data.score.max,7).toString());result=scorm.setvalue(p1+'success_status',data.success_status);result=scorm.setvalue(p1+'completion_status',data.completion_status);result=scorm.setvalue(p1+'progress_measure',trueRound(data.progress_measure,7).toString());result=scorm.setvalue(p1+'description',data.description);}else{scorm.debug(settings.prefix+": Developer, you didn't pass a score object!",1);}}else{p1+=n+'.';if(!isBadValue(data.score.scaled)){result=scorm.setvalue(p1+'score.scaled',trueRound(data.score.scaled,7).toString());}
...
if(!isBadValue(data.score.max)){result=scorm.setvalue(p1+'score.max',trueRound(data.score.max,7).toString());}
...
return notStartedYet();};this.getObjective=function(id){if(isStarted){var n=scorm.getObjectiveByID(id),p1='cmi.objectives.',obj={};if(n==='false'){return n;}
...
return notStartedYet();};this.happyEnding=function(){if(isStarted){scorm.setvalue('cmi.score.scaled','1');scorm.setvalue('cmi.score.raw','1');scorm.setvalue('cmi.success_status','passed');return scorm.setvalue('cmi.completion_status','completed');}
