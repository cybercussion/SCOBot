...
return!(o.constructor&&!hasOwn.call(o.constructor.prototype,"isPrototypeOf"));},isArray=function(o){return(o instanceof Array)||(toString.apply(o)==='[object Array]');},isFunction=function(o){return type(o)==="function";},extend=function(o){var i=1,args=arguments,len=args.length,key;while(i<len){for(key in args[i]){if(args[i].hasOwnProperty(key)){args[0][key]=args[i][key];}}
i+=1;}
return args[0];},addEvent=function(target,event,handler){if(target===window||isElement(target)){if(target.addEventListener){if(event==="loaded"){event="DOMContentLoaded";}
target.addEventListener(event,handler,false);}else{if(event==="loaded"){event="onreadystatechange";document.attachEvent(event,function(){if(document.readyState==="complete"){document.detachEvent("onreadystatechange",target);isReady=true;handler();}});if(document.documentElement.doScroll&&window===window.top){checkLoaded(handler);}}else{target.attachEvent('on'+event,handler);}}}else{extend(target,Events);target.on(event,handler);}},triggerEvent=function(target,name,options){var doc=document,event,type,attr;options=options||{};for(attr in defaults){if(!options.hasOwnProperty(attr)){options[attr]=defaults[attr];}}
if(isWindow(target)||isElement(target)){if(doc.createEvent){type=eventTypes[name]||'CustomEvent';event=doc.createEvent(type);initializers[type](target,name,event,options);try{target.dispatchEvent(event);}catch(e){}}else{event=doc.createEventObject();target.fireEvent('on'+type,event);}}else{try{target.trigger(name,options);}catch(ee){}}},triggerEvents=function(events,args){var ev,i=-1,l=events.length,a1=args[0],a2=args[1],a3=args[2],al=args.length;switch(al){case 0:while(++i<l)(ev=events[i]).callback.call(ev.ctx);return;case 1:while(++i<l)(ev=events[i]).callback.call(ev.ctx,a1);return;case 2:while(++i<l)(ev=events[i]).callback.call(ev.ctx,a1,a2);return;case 3:while(++i<l)(ev=events[i]).callback.call(ev.ctx,a1,a2,a3);return;default:while(++i<l)(ev=events[i]).callback.apply(ev.ctx,args);return;}},eventsApi=function(obj,action,name,rest){if(!name){return true;}
...
return"false";};this.Commit=function(){scorm.debug(settings.prefix+": Commit called.\nSuspend Data Usage "+suspendDataUsageStatistic(),4);$.triggerEvent(self,'StoreData',{name:'StoreData',runtimedata:cmi});return'true';};this.Terminate=function(){settings.terminated=1;settings.initialized=0;return'true';};this.GetErrorString=function(param){if(param!==""){var nparam=parseInt(param,10);if(errors[nparam]!==undefined){return errors[nparam];}}
return"";};this.GetLastError=function(){return settings.errorCode;};this.GetDiagnostic=function(){return settings.diagnostic;};}
function SCOBotBase(options){"use strict";var $=SCOBotUtil,defaults={version:"4.0.0",createDate:"04/05/2011 08:56AM",modifiedDate:"07/23/2014 10:40AM",debug:false,isActive:false,throw_alerts:false,prefix:"SCOBotBase",exit_type:"suspend",success_status:"unknown",use_standalone:true,standalone:false,completion_status:"unknown",time_type:"GMT",cmi:null},settings=$.extend(defaults,options),isError=0,error={0:"No Error",404:"Not Found",405:"Prevented on a read only resource"},API={connection:false,version:"none",mode:"",path:false,data:{completion_status:settings.completion_status,success_status:settings.success_status,exit_type:settings.exit_type},isActive:settings.isActive},self=this;settings.error=error;settings.startDate={};function noconsole(msg,lvl){$.triggerEvent(self,debug,{msg:msg,lvl:lvl});}
...
function scorm12toMS(n){var t_arr=[];t_arr=n.split(":");return Math.round(t_arr[0]*3600000)+(t_arr[1]*60000)+(t_arr[2]*1000);}
...
function triggerException(msg){$.triggerEvent(self,'exception',{'error':msg});}
...
ec=getLastErrorCode();m=getLastErrorMessage(ec);d=getDiagnostic(ec);$.triggerEvent(self,'getvalue',{'n':n,'v':v,'error':{'code':ec,'message':m,'diagnostic':d}});if(ec===0||ec===403){if(v==='undefined'||v===null||v==='null'){v="";}
if(API.version==="1.2"){if(v==="0000:00:00.0"){v='';}}
return String(v);}
debug(settings.prefix+": Error\nError Code: "+ec+"\nError Message: "+m+"\nDiagnostic: "+d,1);return'false';}
debug(settings.prefix+": "+n+" Get Aborted, connection not initialized! "+API.isActive,2);return'false';};this.setvalue=function(n,v){var s='false',lms=API.path,ec=0,m='',d='',tiers=[],nn=null,ig=false;if(API.isActive){switch(API.version){case"1.2":API.mode=API.mode===""?lms.LMSGetValue('cmi.core.lesson_mode'):API.mode;if(API.mode==="normal"){switch(n){case"cmi.score.scaled":case"cmi.progress_measure":ig=true;break;case"cmi.comments_from_learner.comment":if(v.length>4096){debug(settings.prefix+": Warning, the comment is over the limit!!",2);}
nn="cmi.comments";break;case"cmi.location":if(v.length>255){debug(settings.prefix+": Warning, your bookmark is over the limit!!",2);}
nn="cmi.core.lesson_location";break;case"cmi.mode":nn="cmi.core.lesson_mode";break;case"cmi.exit":nn="cmi.core.exit";API.exit_type=v;break;case"cmi.score.raw":nn="cmi.core.score.raw";break;case"cmi.score.min":nn="cmi.core.score.min";break;case"cmi.score.max":nn="cmi.core.score.max";break;case"cmi.success_status":case"cmi.completion_status":nn="cmi.core.lesson_status";API.data.completion_status=v;break;case"cmi.scaled_passing_score":nn="cmi.student_data.mastery_score";break;case"cmi.learner_preferences.audio_level":nn="cmi.student_preferences.audio";break;case"cmi.learner_preferences.delivery_speed":nn="cmi.student_preferences.speed";break;case"cmi.learner_preferences.language":nn="cmi.student_preferences.language";break;case"cmi.learner_preferences.audio_captioning":nn="cmi.student_preferences.text";break;case"cmi.session_time":nn="cmi.core.session_time";break;case"cmi.total_time":nn="cmi.core.total_time";break;case"cmi.suspend_data":if(v.length>4096){debug(settings.prefix+": Warning, your suspend data is over the limit!!",2);}
...
break;default:break;}
ec=getLastErrorCode();m=getLastErrorMessage(ec);d=getDiagnostic(ec);$.triggerEvent(self,'setvalue',{'n':n,'v':v,'error':{'code':ec,'message':m,'diagnostic':d}});if(ec===0||ec===403){return s;}
...
function SCOBot(options){"use strict";var $=SCOBotUtil,defaults={version:"4.0.0",createDate:"04/07/2011 09:33AM",modifiedDate:"07/23/2014 10:51PM",prefix:"SCOBot",launch_data:{},interaction_mode:"state",launch_data_type:"querystring",initiate_timer:true,scorm_strict:true,scorm_edition:"3rd",success_status:"unknown",scorm_status_persist:"success_status",location:"",completion_status:"",suspend_data:{pages:[]},mode:"",completion_threshold:0,scaled_passing_score:0.7,max_time_allowed:'',totalInteractions:0,totalObjectives:0,startTime:0},settings=$.extend(defaults,options),lmsconnected=false,isError=false,isStarted=false,badValues='|null|undefined|false|NaN|| |',error=scorm.get('error'),self=this;function initSCO(){lmsconnected=scorm.initialize();scorm.debug(settings.prefix+": SCO Loaded from window.onload "+lmsconnected,4);if(lmsconnected){self.start();$.triggerEvent(self,"load");}
return lmsconnected;}
function exitSCO(){scorm.debug("SCO is being asked, *cough* forced to exit ...",3);if(isStarted){$.triggerEvent(self,"unload");switch(scorm.get('exit_type')){case"finish":self.finish();break;case"suspend":self.suspend();break;case"timeout":self.timeout();break;default:scorm.debug(settings.prefix+": unknown exit type",2);break;}
...
return reg.exec(str);}
function timesUp(){scorm.debug("Times Up!");var time_action=scorm.getvalue('cmi.time_limit_action').split(','),message=!!((time_action[1]==="message"));if(message){$.triggerEvent(self,'message',{text:"Time Limit Exceeded"});}
scorm.set('exit_type',"timeout");if(time_action[0]==="exit"){exitSCO();}else{$.triggerEvent(self,'continue');}}
...
return value;}
scorm.debug(settings.prefix+": Developer, you're not passing true or false for true-false.  I got "+value+" instead",1);return'';case'choice':if(scorm.getAPIVersion()==="1.2"){if($.isArray(value)){if(value.length>26&&settings.scorm_strict){scorm.debug(settings.prefix+": Developer, you're passing a sum of values that exceeds SCORMs limit of 26 for this pattern.  Consider using 'performance' instead.",2);}
return value.join(",");}
...
case'sequencing':if($.isArray(value)){index=0;if(value.length>36&&settings.scorm_strict){scorm.debug(settings.prefix+": Developer, you're passing a sum of values that exceeds SCORMs limit of 36 for this pattern.",2);value=value.slice(0,36);}
...
if($.isArray(value.words)){str+=value.words.join("[,]");}else{scorm.debug(settings.prefix+": Developer, expected an array of word(s) for fill-in.  I got "+typeof value.words+" instead",1);}
...
return value;case'matching':if($.isArray(value)){len=value.length;i=0;while(i<len){if($.isArray(value[i])){arr.push(scorm.getAPIVersion()==="1.2"?value[i].join("."):value[i].join("[.]"));}else{scorm.debug(settings.prefix+": Developer, you're not passing a array type for matching/performance.  I got "+typeof value+" instead",1);return'';}
i+=1;}
...
if($.isArray(value.answers)){len=value.answers.length;i=0;while(i<len){if($.isArray(value.answers[i])){if($.isPlainObject(value.answers[i][1])){arr2=[trueRound(value.answers[i][1].min,7),trueRound(value.answers[i][1].max,7)];str2=arr2.join("[:]");value.answers[i][1]=str2;}
...
count=parseInt(scorm.getvalue('cmi.objectives._count'),10);scorm.debug(settings.prefix+": Objectives Count is "+count);if(count>0){count=count-1;i=count;while(i>=0){tmpRaw=parseFloat(scorm.getvalue('cmi.objectives.'+i+'.score.raw'));scorm.debug(settings.prefix+': Score Raw: '+tmpRaw);if(!isNaN(tmpRaw)){scoreRaw+=parseFloat(tmpRaw);}else{scorm.debug(settings.prefix+": We got a NaN converting objectives."+i+".score.raw.  This may be a global/local objective via the imsmanifest.xml.",2);}
...
settings.completion_status=scorm.getvalue('cmi.completion_status');settings.success_status=scorm.getvalue('cmi.success_status');settings.comments_from_lms=getCommentsFromLMS();if(settings.comments_from_lms!=='false'){$.triggerEvent(self,'comments_lms',{data:settings.comments_from_lms});}
...
return notStartedYet();};this.progress=checkProgress;this.setSuspendDataByPageID=function(id,title,data){if(isStarted){var i=0,len=settings.suspend_data.pages.length;while(i<len){if(settings.suspend_data.pages[i].id===id){settings.suspend_data.pages[i].data=data;scorm.debug(settings.prefix+": Suspend Data Set",4);scorm.debug(settings.suspend_data,4);return setSuspendData();}
i+=1;}
...
return notStartedYet();};this.getSuspendDataByPageID=function(id){if(isStarted){var i=0,len=settings.suspend_data.pages.length;while(i<len){if(settings.suspend_data.pages[i].id===id){return settings.suspend_data.pages[i].data;}
i+=1;}
return'false';}
return notStartedYet();};this.getSecondsFromStart=function(){return settings.startTime-currentTime();};this.setInteraction=function(data){if(isStarted){var version=scorm.getAPIVersion(),n,m,i,j,p,p1='cmi.interactions.',p2,orig_timestamp=data.timestamp||scorm.isoStringToDate(scorm.getvalue(p1+scorm.getInteractionByID(data.id)+'.timestamp')),timestamp,latency,result,len,key;if(!$.isPlainObject(data)){scorm.debug(settings.prefix+": Developer, your not passing a {object} argument!!  Got "+typeof data+" instead.",1);return'false';}
...
return'false';}
if($.type(data.timestamp)==="date"){timestamp=scorm.getAPIVersion()==="1.2"?scorm.dateToscorm12Time(data.timestamp):scorm.isoDateToString(data.timestamp);}
...
p2='correct_responses._count';if($.isArray(data.correct_responses)){j=0;len=data.correct_responses.length;while(j<len){p=scorm.getInteractionCorrectResponsesByPattern(n,data.correct_responses[j].pattern);scorm.debug(settings.prefix+": Trying to locate pattern "+data.correct_responses[j].pattern+" resulted in "+p,4);if(p==='false'){p=scorm.getvalue(p1+p2)==='-1'?0:scorm.getvalue(p1+p2);scorm.debug(settings.prefix+": p is now "+p,4);}
...
return'false';}}
if($.isPlainObject(data.score)){if(version==="2004"){result=!isBadValue(data.score.scaled)?sv(p1+'score.scaled',trueRound(data.score.scaled,7).toString()):scorm.debug(settings.prefix+def1+p1+"score.scaled: "+data.score.scaled+def2,3);}
...
return notStartedYet();};this.isISO8601=isISO8601;this.getAPIVersion=scorm.getAPIVersion;this.set=function(n,v){switch(n){case"version":case"createDate":case"modifiedDate":case"prefix":triggerWarning(405);break;default:settings[n]=v;break;}
return(isError===false);};this.get=function(n){if(settings[n]===undefined){triggerWarning(404);return false;}
return settings[n];};$.addEvent(window,'loaded',initSCO);$.addEvent(window,'unload',exitSCO);$.addEvent(scorm,'exception',function(e){triggerException(e.error);});}