...
return'false';}
function checkProgress(){if(isStarted){var scoreRaw=0,tmpRaw=0,scoreScaled=1,progressMeasure,completionStatus=scorm.getvalue('cmi.completion_status'),totalObjectivesCompleted=0,i=0,count;if(settings.totalInteractions===0||settings.totalObjectives===0){scorm.debug(settings.prefix+": Sorry, I cannot calculate Progress as the totalInteractions and or Objectives are zero",2);return'false';}
...
progressMeasure=(totalObjectivesCompleted/settings.totalObjectives).toString();scorm.setvalue('cmi.progress_measure',trueRound(progressMeasure,7));if(completionStatus==="not attempted"||completionStatus==="unknown"){settings.completion_status=(parseFloat(progressMeasure)>=parseFloat(settings.completion_threshold))?'completed':'incomplete';scorm.setvalue('cmi.completion_status',settings.completion_status);}else{settings.completion_status=completionStatus;}
scorm.debug(settings.prefix+": Pass/Fail check - Calculated scaled score:"+parseFloat(scoreScaled)+" vs. "+parseFloat(settings.scaled_passing_score),3);settings.success_status=(parseFloat(scoreScaled)>=parseFloat(settings.scaled_passing_score))?'passed':'failed';scorm.setvalue('cmi.success_status',settings.success_status);return{score_scaled:scorm.getvalue('cmi.score.scaled'),success_status:scorm.getvalue('cmi.success_status'),progress_measure:scorm.getvalue('cmi.progress_measure'),completion_status:scorm.getvalue('cmi.completion_status')};}
...
tmpScaledPassingScore=scorm.getvalue('cmi.scaled_passing_score');if(!isBadValue(tmpScaledPassingScore)&&tmpScaledPassingScore!=="-1"){if(parseFloat(tmpScaledPassingScore>1)){tmpScaledPassingScore=((parseFloat(tmpScaledPassingScore)*10)/1000).toString();}
settings.scaled_passing_score=tmpScaledPassingScore;}
settings.completion_status=scorm.getvalue('cmi.completion_status');settings.success_status=scorm.getvalue('cmi.success_status');settings.comments_from_lms=getCommentsFromLMS();if(settings.comments_from_lms!=='false'){Utl.triggerEvent(self,'comments_lms',{data:settings.comments_from_lms});}
...
return notStartedYet();};this.gradeIt=function(){var scoreScaled=1,scoreRaw=scorm.getvalue('cmi.score.raw'),scoreMin=scorm.getvalue('cmi.score.min'),scoreMax=scorm.getvalue('cmi.score.max'),progressMeasure=scorm.getvalue('cmi.progress_measure'),completionStatus=scorm.getvalue('cmi.completion_status');if((scoreMax-scoreMin)===0){scorm.debug(settings.prefix+": Division by Zero for scoreMax - scoreMin "+scoreMax,2);scorm.setvalue('cmi.score.scaled',scoreScaled);}else{scoreScaled=((scoreRaw-scoreMin)/(scoreMax-scoreMin)).toString();scorm.debug(settings.prefix+": Score Scaled = "+scoreScaled,3);scorm.setvalue('cmi.score.scaled',trueRound(scoreScaled,7));}
if(completionStatus==="unknown"||completionStatus==="not attempted"){settings.completion_status=(parseFloat(progressMeasure)>=parseFloat(settings.completion_threshold))?'completed':'incomplete';scorm.setvalue('cmi.completion_status',settings.completion_status);}else{settings.completion_status=completionStatus;}
settings.success_status=(parseFloat(scoreScaled)>=parseFloat(settings.scaled_passing_score))?'passed':'failed';scorm.setvalue('cmi.success_status',settings.success_status);return'true';};this.happyEnding=function(){if(isStarted){scorm.setvalue('cmi.score.scaled','1');scorm.setvalue('cmi.score.min','0');scorm.setvalue('cmi.score.max','1');scorm.setvalue('cmi.score.raw','1');scorm.setvalue('cmi.success_status','passed');scorm.setvalue('cmi.progress_measure','1');return scorm.setvalue('cmi.completion_status','completed');}
