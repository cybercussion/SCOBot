...
return str;}
function ISODurationToCentisec(str){var aV=[0,0,0,0,0,0],bErr=!!((str.indexOf("P")!==0)),bTFound=false,aT=["Y","M","D","H","M","S"],p=0,i=0;if(!bErr){str=str.substr(1);for(i=0;i<aT.length;i+=1){if(str.indexOf("T")===0){str=str.substr(1);i=Math.max(i,3);bTFound=true;}
...
if(isNaN(aV[i])){bErr=true;break;}else if((i>2)&&(!bTFound)){bErr=true;break;}
str=str.substr(p+1);}}
...
ec=getLastErrorCode();if(ec===0||ec===403){if(v==='undefined'||v===null||v==='null'){v="";}
return String(v);}
...
break;default:break;}
ec=getLastErrorCode();if(ec===0||ec===403){return s;}
...
return'false';};this.init=function(){var win;try{win=window.parent;if(win&&win!==window){findAPI(window.parent);}}catch(e){}
if(!API.path){try{win=window.top.opener;findAPI(win);}catch(ee){}}
...
function encodeInteractionType(type,value){var str='',str2='',i=0,arr=[],arr2=[],index;switch(type){case'true-false':value=value.toString();if(value==='true'||value==='false'){return value;}
...
if($.isArray(value.answers)){for(i=0;i<value.answers.length;i+=1){if($.isArray(value.answers[i])){if($.isPlainObject(value.answers[i][1])){arr2=[trueRound(value.answers[i][1].min,7),trueRound(value.answers[i][1].max,7)];str2=arr2.join("[:]");value.answers[i][1]=str2;}
...
return'false';}
return notStartedYet();};this.getSecondsFromStart=function(){return settings.startTime-currentTime();};this.setInteraction=function(data){if(isStarted){var n,m,i,j,p,p1='cmi.interactions.',p2,orig_timestamp=data.timestamp||scorm.isoStringToDate(scorm.getvalue(p1+scorm.getInteractionByID(data.id)+'.timestamp')),timestamp,latency,result,key;if(!$.isPlainObject(data)){scorm.debug(settings.prefix+": Developer, your not passing a {object} argument!!  Got "+typeof data+" instead.",1);return'false';}
