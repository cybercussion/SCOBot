...
setData(k.substr(4,k.length),v,cmi);cmi.comments_from_learner._count=(getObjLength(cmi.comments_from_learner)-2).toString();return'true';break;case"interactions":if(cmi.interactions._children.indexOf(tiers[3])===-1){return throwVocabError(key,v);}
...
return'true';}else{scorm.debug("Can't add interaction without ID first!",3);return'false';}}else{if(tiers[3]==='objectives'){if(tiers[5]==="id"){count=parseInt(cmi.interactions[tiers[2]].objectives._count,10);for(z=0;z<count;z+=1){if(cmi.interactions[tiers[2]].objectives[z].id===v){return throwGeneralSetError(key,v,z);}}}else{return throwVocabError(key,v);}
...
setData(k.substr(4,k.length),v,cmi);cmi.interactions._count=(getObjLength(cmi.interactions)-2).toString();return'true';}
break;case"objectives":if(tiers[3]==="id"){count=parseInt(cmi.objectives._count,10);for(z=0;z<count;z+=1){if(cmi.objectives[z].id===v){settings.errorCode="351";settings.diagnostic="The objectives.id element must be unique.  The value '"+v+"' has already been set in objective #"+z;return'false';}}}
...
setData(k.substr(4,k.length),v,cmi);cmi.objectives._count=(getObjLength(cmi.objectives)-2).toString();return'true';if(isNaN(parseInt(tiers[2],10))){return'false';}
break;}
break;}
...
return str;}
function ISODurationToCentisec(str){var aV=[0,0,0,0,0,0],bErr=!!((str.indexOf("P")!==0)),bTFound=false,aT=["Y","M","D","H","M","S"],p=0,i=0;if(!bErr){str=str.substr(1);for(i=0;i<aT.length;i++){if(str.indexOf("T")==0){str=str.substr(1);i=Math.max(i,3);bTFound=true;}
p=str.indexOf(aT[i]);if(p>-1){if((i==1)&&(str.indexOf("T")>-1)&&(str.indexOf("T")<p)){continue;}
if(aT[i]=="S"){aV[i]=parseFloat(str.substr(0,p))}else{aV[i]=parseInt(str.substr(0,p))}
...
if(!API.path){try{win=window.top.opener;findAPI(win);}catch(e){}}
...
scorm.debug(settings.prefix+": Developer, you're not passing true or false for true-false.  I got "+value+" instead",1);return'';case'choice':case'sequencing':if($.isArray(value)){var index=0;if(value.length>36&&settings.scorm_strict){scorm.debug(settings.prefix+": Developer, you're passing a sum of values that exceeds SCORM's limit of 36 for this pattern.",2);value=value.slice(0,36);}
...
return'false';}
return notStartedYet();};this.getSecondsFromStart=function(){return settings.startTime-currentTime();};this.setInteraction=function(data){if(isStarted){var n,m,i,j,p,p1='cmi.interactions.',p2,orig_timestamp=data.timestamp||scorm.isoStringToDate(scorm.getvalue(p1+scorm.getInteractionByID(data.id)+'.timestamp')),timestamp,latency,result,cr,cr_hash='';if(!$.isPlainObject(data)){scorm.debug(settings.prefix+": Developer, your not passing a {object} argument!!  Got "+typeof(data)+" instead.",1);return'false';}
if(isBadValue(data.id)){scorm.debug(settings.prefix+": Developer, your passing a interaction without a ID\nSee question:\n"+data.description,1);for(var key in data){if(data.hasOwnProperty(key)){scorm.debug("key: "+key+"\n value: "+data[key]);}}
return'false';}
...
return notStartedYet();};this.setObjective=function(data){if(isStarted){var p1='cmi.objectives.',n=scorm.getObjectiveByID(data.id),result='false',f=false,def1=": Passed no or bad ",def2=" ignored.",sv=scorm.setvalue;scorm.debug(settings.prefix+": Setting Objective at "+n+" (This may be false)");if(isBadValue(n)){n=scorm.getvalue(p1+'_count');if(n==='false'){scorm.debug(settings.prefix+": LMS is return false, can not proceed, check error codes");return n;}
scorm.debug(settings.prefix+": Objective "+data.id+" was not found.  Adding new at "+n+" "+data.description);f=true;}
p1+=n+'.';if(f){if(!isBadValue(data.id)){sv(p1+'id',data.id.toString());}else{scorm.debug(settings.prefix+": You did not pass an objective ID!!  What I did get below:",1);for(var key in data){if(data.hasOwnProperty(key)){scorm.debug("key: "+key+"\n value: "+data[key]);}}
return'false';}}
