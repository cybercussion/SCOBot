...
if(!$.isPlainObject(cmi.interactions[tiers[2]])){if(tiers[3]==="id"){cmi.interactions[tiers[2]]={};setData(k.substr(4,k.length),v,cmi);cmi.interactions._count=(getObjLength(cmi.interactions)-2).toString();if(!$.isPlainObject(cmi.interactions[tiers[2]].objectives)){scorm.debug(settings.prefix+": Constructing objectives object for new interaction",4);cmi.interactions[tiers[2]].objectives={};cmi.interactions[tiers[2]].objectives._count="-1";cmi.interactions[tiers[2]].objectives._children="id,score,success_status,completion_status,description";}
...
return'true';}else{scorm.debug("Can't add interaction without ID first!",3);return'false';}}else{if(tiers[3]==='objectives'){if(cmi.interactions[tiers[2]].objectives._children.indexOf(tiers[5])===-1){return throwVocabError(key,v);}
if(tiers[5]==="id"){count=parseInt(cmi.interactions[tiers[2]].objectives._count,10);for(z=0;z<count;z+=1){if(cmi.interactions[tiers[2]].objectives[z].id===v){return throwGeneralSetError(key,v,z);}}}
setData(k.substr(4,k.length),v,cmi);cmi.interactions[tiers[2]].objectives._count=(getObjLength(cmi.interactions[tiers[2]].objectives)-2).toString();return'true';}
...
return str;}
function ISODurationToCentisec(str){var aV=new Array(0,0,0,0,0,0),bErr=!!((str.indexOf("P")!==0)),bTFound=false,aT=new Array("Y","M","D","H","M","S"),p=0,i=0;if(!bErr){str=str.substr(1);for(i=0;i<aT.length;i++){if(str.indexOf("T")==0){str=str.substr(1);i=Math.max(i,3);bTFound=true;}
