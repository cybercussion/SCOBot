...
i+=1;}
return args[0];},addEvent=function(target,event,handler){if(event.indexOf(' ')>=0){var events=event.split(' '),len=events.length;while(len--){addEvent(target,events[len],handler);}}else{if(target===window||isElement(target)){if(target.addEventListener){if(event==="loaded"){event="DOMContentLoaded";}
target.addEventListener(event,handler,false);}else{if(event==="loaded"){event="onreadystatechange";document.attachEvent(event,function(){if(document.readyState==="complete"){document.detachEvent("onreadystatechange",target);isReady=true;handler();}});if(document.documentElement.doScroll&&window===window.top){checkLoaded(handler);}}else{target.attachEvent('on'+event,handler);}}}else{extend(target,Events);target.on(event,handler);}}},calcAverage=function(num_arr){var sum=0,i=0,len=num_arr.length;while(i<len){sum+=num_arr[i].lat;i+=1;}
sum=sum/len;return sum.toFixed(2);},triggerEvent=function(target,name,options){var doc=document,event,type,attr;options=options||{};for(attr in defaults){if(!options.hasOwnProperty(attr)){options[attr]=defaults[attr];}}
...
ec=getLastErrorCode();m=getLastErrorMessage(ec);d=getDiagnostic(ec);Utl.triggerEvent(self,'getvalue',{'n':n,'v':v,'error':{'code':ec,'message':m,'diagnostic':d}});end=new Date();settings.latency_arr.push({lat:end-start,v:n});if(ec===0||ec===403){if(v==='undefined'||v===null||v==='null'){v="";}
...
break;default:break;}
nn=tiers.join(".");break;case"objectives":switch(tiers[3]){case"id":break;case"status":case"success_status":case"completion_status":tiers[3]='status';if(v==="unknown"||v==="not attempted"){ig=true;}
...
debug(settings.prefix+": "+n+" Set Aborted, connection not initialized! Locate where you called it after you Terminated.",2);return'false';};this.commit=function(){var s='false',lms=API.path,ec=0,session_secs,saveDate=new Date();settings.latency_arr=[];session_secs=(saveDate.getTime()-settings.startDate.getTime())/1000;if(API.isActive){debug(settings.prefix+": Committing data",3);switch(API.version){case"1.2":self.setvalue("cmi.core.session_time",centisecsToSCORM12Duration(session_secs*100));s=lms.LMSCommit("");break;case"2004":self.setvalue("cmi.session_time",centisecsToISODuration(session_secs*100,true));s=lms.Commit("");break;default:break;}
...
function SCOBot(options){"use strict";var Utl=SCOBotUtil,defaults={version:"4.0.4",createDate:"04/07/2011 09:33AM",modifiedDate:"12/04/2014 10:24AM",prefix:"SCOBot",launch_data:{},interaction_mode:"state",launch_data_type:"querystring",initiate_timer:true,scorm_strict:true,scorm_edition:"3rd",scorm_status_persist:"success_status",location:"",useJSONSuspendData:true,suspend_data:{pages:[]},base64:false,happyEnding:true,mode:"",scaled_passing_score:0.7,completion_threshold:0,max_time_allowed:'',totalInteractions:0,totalObjectives:0,startTime:0},settings=Utl.extend(defaults,options),buffer={success_status:'',completion_status:'',completion_threshold:settings.completion_threshold,progress_measure:0,scaled_passing_score:settings.scaled_passing_score,score:{scaled:0,raw:0,min:0,max:0}},lmsconnected=false,isError=false,isStarted=false,badValues='|null|undefined|false|NaN|| |',error=scorm.get('error'),self=this;function initSCO(){lmsconnected=scorm.initialize();scorm.debug(settings.prefix+": SCO Loaded from window.onload "+lmsconnected,4);if(lmsconnected){self.start();Utl.triggerEvent(self,"load");}
return lmsconnected;}
function exitSCO(){scorm.debug("SCO is being asked, *cough* forced to exit ...",3);if(isStarted){Utl.triggerEvent(self,"unload");switch(scorm.get('exit_type')){case"finish":self.finish();break;case"suspend":self.suspend();break;case"timeout":self.timeout();break;default:scorm.debug(settings.prefix+": unknown exit type",2);break;}
...
value=str;return value;case'numeric':if(typeof value==="number"){str=value.toString();}else if(Utl.isPlainObject(value)){arr=[trueRound(value.min,7),trueRound(value.max,7)];str=arr.join("[:]");}else{str=parseFloat(value);if(str==="NaN"){scorm.debug(settings.prefix+": Developer, your not passing a number for a numeric interaction.  I got "+value+" instead",1);}
str+='';}
...
function setSuspendData(){var result,cleansedData=cleanseData(JSON.stringify(settings.suspend_data)),data=settings.base64?window.btoa(cleansedData):cleansedData;result=scorm.setvalue('cmi.suspend_data',data);if(result==='true'){scorm.debug(settings.prefix+": Suspend Data saved",4);scorm.debug(settings.suspend_data,4);return'true';}
return'false';}
function checkProgress(){if(isStarted){var tmpRaw=0,completionStatus=scorm.getvalue('cmi.completion_status'),totalObjectivesCompleted=0,i=0,count;buffer.score.raw=0;if(settings.totalInteractions===0||settings.totalObjectives===0){scorm.debug(settings.prefix+": Sorry, I cannot calculate Progress as the totalInteractions and or Objectives are zero",2);return'false';}
...
i-=1;}}
scorm.debug(settings.prefix+" Setting score "+scorm.setvalue('cmi.score.raw',buffer.score.raw.toString()));if((buffer.score.max-buffer.score.min)===0){scorm.debug(settings.prefix+": Division by Zero for scoreMax - scoreMin "+buffer.score.max,2);scorm.setvalue('cmi.score.scaled','1');}else{buffer.score.scaled=((buffer.score.raw-buffer.score.min)/(buffer.score.max-buffer.score.min)).toString();scorm.debug(settings.prefix+": Score Scaled = "+buffer.score.scaled,3);scorm.setvalue('cmi.score.scaled',trueRound(buffer.score.scaled,7));}
buffer.progress_measure=(totalObjectivesCompleted/settings.totalObjectives).toString();scorm.setvalue('cmi.progress_measure',trueRound(buffer.progress_measure,7));buffer.completion_status=(parseFloat(buffer.progress_measure)>=parseFloat(buffer.completion_threshold))?'completed':'incomplete';scorm.setvalue('cmi.completion_status',buffer.completion_status);scorm.debug(settings.prefix+": Pass/Fail check - Calculated scaled score:"+parseFloat(buffer.score.scaled)+" vs. "+parseFloat(buffer.scaled_passing_score),3);buffer.success_status=(parseFloat(buffer.score.scaled)>=parseFloat(buffer.scaled_passing_score))?'passed':'failed';scorm.setvalue('cmi.success_status',buffer.success_status);return{score_scaled:buffer.score.scaled,success_status:buffer.success_status,progress_measure:buffer.progress_measure,completion_status:buffer.completion_status};}
...
return response;}
return notStartedYet();}
function updateStatus(ending){var ss='cmi.success_status',cs='cmi.completion_status',defss=scorm.get('success_status'),defcs=scorm.get('completion_status'),storss=self.getvalue(ss),storcs=self.getvalue(cs),isSuccessSet=false,isCompletionSet=false;if(storss==="passed"||storss==="failed"){isSuccessSet=true;}
...
tmpScaledPassingScore=scorm.getvalue('cmi.scaled_passing_score');if(!isBadValue(tmpScaledPassingScore)&&tmpScaledPassingScore!=="-1"){if(parseFloat(tmpScaledPassingScore>1)){buffer.scaled_passing_score=((parseFloat(tmpScaledPassingScore)*10)/1000).toString();}
...
if(!isBadValue(data.scoreMin)){buffer.score.min=trueRound(data.scoreMin,7);scorm.setvalue('cmi.score.min',buffer.score.min.toString());}
if(!isBadValue(data.scoreMax)){buffer.score.max=trueRound(data.scoreMax,7);scorm.setvalue('cmi.score.max',buffer.score.max.toString());}
return'true';}
...
i+=1;}
return'false';}
return notStartedYet();};this.getSecondsFromStart=function(){return settings.startTime-currentTime();};this.setInteraction=function(data){if(isStarted){var version=scorm.getAPIVersion(),n,m,i,j,p,p1='cmi.interactions.',p2,orig_timestamp=data.timestamp||scorm.isoStringToDate(scorm.getvalue(p1+scorm.getInteractionByID(data.id)+'.timestamp')),timestamp,latency,result,len,key;if(!Utl.isPlainObject(data)){scorm.debug(settings.prefix+": Developer, your not passing a {object} argument!!  Got "+typeof data+" instead.",1);return'false';}
...
return notStartedYet();};this.gradeIt=function(){var scoreScaled=1,scoreRaw=scorm.getvalue('cmi.score.raw'),scoreMin=scorm.getvalue('cmi.score.min'),scoreMax=scorm.getvalue('cmi.score.max');if((scoreMax-scoreMin)===0){scorm.debug(settings.prefix+": Division by Zero for scoreMax - scoreMin "+scoreMax,2);scorm.setvalue('cmi.score.scaled',scoreScaled);}else{scoreScaled=((scoreRaw-scoreMin)/(scoreMax-scoreMin)).toString();scorm.debug(settings.prefix+": Score Scaled = "+scoreScaled,3);scorm.setvalue('cmi.score.scaled',trueRound(scoreScaled,7));}
if(settings.completion_status!=="completed"){settings.completion_status=(parseFloat(progressMeasure)>=parseFloat(settings.completion_threshold))?'completed':'incomplete';scorm.setvalue('cmi.completion_status',settings.completion_status);}
if(settings.success_status!=="passed"){settings.success_status=(parseFloat(scoreScaled)>=parseFloat(settings.scaled_passing_score))?'passed':'failed';}
scorm.setvalue('cmi.success_status',settings.success_status);return'true';};this.happyEnding=function(){if(isStarted&&settings.happyEnding){scorm.setvalue('cmi.score.scaled','1');scorm.setvalue('cmi.score.min','0');scorm.setvalue('cmi.score.max','100');scorm.setvalue('cmi.score.raw','100');scorm.setvalue('cmi.success_status','passed');scorm.setvalue('cmi.progress_measure','1');return scorm.setvalue('cmi.completion_status','completed');}
return notStartedYet();};this.commit=function(){if(isStarted){return scorm.commit('');}
return notStartedYet();};this.suspend=function(){if(isStarted){scorm.debug(settings.prefix+": I am suspending...",3);scorm.setvalue('cmi.exit','suspend');isStarted=false;return scorm.terminate();}
return notStartedYet();};this.finish=function(){if(isStarted){scorm.debug(settings.prefix+": I am finishing...",3);scorm.setvalue('cmi.exit','normal');updateStatus(true);isStarted=false;return scorm.terminate();}
return notStartedYet();};this.timeout=function(){if(isStarted){scorm.debug(settings.prefix+": I am timing out...",3);scorm.setvalue('cmi.exit','time-out');updateStatus(true);isStarted=false;return scorm.terminate();}
