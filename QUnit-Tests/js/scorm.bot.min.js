...
function SCOBot(options){"use strict";var Utl=SCOBotUtil,defaults={version:"4.0.10",createDate:"04/07/2011 09:33AM",modifiedDate:"05/22/2015 03:26PM",prefix:"SCOBot",launch_data:{},interaction_mode:"state",launch_data_type:"querystring",initiate_timer:true,scorm_strict:true,scorm_edition:"3rd",scorm_status_persist:"success_status",useJSONSuspendData:true,suspend_data:{pages:[]},base64:false,happyEnding:true,doNotStatusUntilFinish:false,location:"",mode:"",scaled_passing_score:0.7,completion_threshold:0,max_time_allowed:'',totalInteractions:0,totalObjectives:0,startTime:0},settings=Utl.extend(defaults,options),buffer={success_status:'',completion_status:'',completion_threshold:settings.completion_threshold,progress_measure:'0',scaled_passing_score:settings.scaled_passing_score,score:{scaled:'0',raw:'0',min:'0',max:'0'}},lmsconnected=false,isError=false,isStarted=false,happyEndingRequest=false,SCOBotManagedStatus=false,badValues='|null|undefined|false|NaN|| |',error=scorm.get('error'),self=this;function initSCO(){lmsconnected=scorm.initialize();scorm.debug(settings.prefix+": SCO Loaded from window.onload "+lmsconnected,4);if(lmsconnected==='true'){self.start();Utl.triggerEvent(self,"load");}else{scorm.debug(settings.prefix+"Sorry, I could not locate an LMS Runtime API.");}
return lmsconnected;}
...
return notStartedYet();};this.finish=function(){if(scorm.isConnectionActive()){scorm.setvalue('cmi.exit','normal');updateStatus(true);isStarted=false;return scorm.terminate();}
