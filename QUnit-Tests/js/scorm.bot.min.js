...
return true;};this.setTotals=function(data){if(scorm.isConnectionActive()){if(!isBadValue(data.totalInteractions)){settings.totalInteractions=data.totalInteractions;}
...
return'true';}
return notStartedYet();};this.startTimer=function(){var time=scorm.ISODurationToCentisec(settings.max_time_allowed)*10;setTimeout(timesUp,time);};this.debug=scorm.debug;this.getvalue=scorm.getvalue;this.setvalue=scorm.setvalue;this.getMode=function(){if(scorm.isConnectionActive()){return settings.mode;}
return notStartedYet();};this.getEntry=function(){if(scorm.isConnectionActive()){return settings.entry;}
return notStartedYet();};this.setBookmark=function(v){if(scorm.isConnectionActive()){settings.location=v.toString();return scorm.setvalue('cmi.location',settings.location);}
...
scorm.setvalue('cmi.success_status',settings.success_status);return'true';};this.happyEnding=function(){if(scorm.isConnectionActive()&&settings.happyEnding){scorm.setvalue('cmi.score.scaled','1');scorm.setvalue('cmi.score.min','0');scorm.setvalue('cmi.score.max','100');scorm.setvalue('cmi.score.raw','100');scorm.setvalue('cmi.success_status','passed');scorm.setvalue('cmi.progress_measure','1');return scorm.setvalue('cmi.completion_status','completed');}
...
return notStartedYet();};this.finish=function(){if(scorm.isConnectionActive()){scorm.debug(settings.prefix+": I am finishing...",3);scorm.setvalue('cmi.exit','normal');updateStatus(true);isStarted=false;return scorm.terminate();}
