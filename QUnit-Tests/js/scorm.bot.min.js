...
debug(settings.prefix+": "+n+" Get Aborted, connection not initialized! "+API.isActive,2);return'false';};this.setvalue=function(n,v){var s='false',lms=API.path,ec=0,m='',d='',tiers=[],nn=null,ig=false;if(API.isActive){switch(API.version){case"1.2":API.mode=API.mode===""?lms.LMSGetValue('cmi.core.lesson_mode'):API.mode;switch(n){case"cmi.score.scaled":case"cmi.progress_measure":ig=true;break;case"cmi.comments_from_learner.comment":if(v.length>4096){debug(settings.prefix+": Warning, the comment is over the limit!!",2);}
...
break;}
if(ig){return'false';}
s=lms.LMSSetValue(nn,v);break;case"2004":API.mode=API.mode===""?lms.GetValue('cmi.mode'):API.mode;if(API.mode==="normal"){switch(n){case"cmi.location":if(v.length>1000){debug(settings.prefix+": Warning, your bookmark is over the limit!!",2);}
...
return settings[n];};Utl.addEvent(window,'loaded',initSCO);Utl.addEvent(window,'onbeforeunload',exitSCO);Utl.addEvent(window,'unload',exitSCO);Utl.addEvent(scorm,'exception',function(e){triggerException(e.error);});this.func=function(){};}