...
if(!$.isPlainObject(cmi.interactions[tiers[2]])){if(tiers[3]==="id"){cmi.interactions[tiers[2]]={};setData(k.substr(4,k.length),v,cmi);cmi.interactions._count=(getObjLength(cmi.interactions)-2).toString();if(!$.isPlainObject(cmi.interactions[tiers[2]].objectives)){scorm.debug(settings.prefix+": Constructing objectives object for new interaction",4);cmi.interactions[tiers[2]].objectives={};cmi.interactions[tiers[2]].objectives._count="-1";}
if(!$.isPlainObject(cmi.interactions[tiers[2]].correct_responses)){scorm.debug(settings.prefix+": Constructing correct responses object for new interaction",4);cmi.interactions[tiers[2]].correct_responses={};cmi.interactions[tiers[2]].correct_responses._count="-1";}
return'true';}
scorm.debug("Can't add interaction without ID first!",3);return'false';}
if(tiers[3]==='objectives'){if(tiers[5]==="id"){count=parseInt(cmi.interactions[tiers[2]].objectives._count,10);for(z=0;z<count;z+=1){if(cmi.interactions[tiers[2]].objectives[z].id===v){return throwGeneralSetError(key,v,z);}}}else{return throwVocabError(key,v);}
...
setData(k.substr(4,k.length),v,cmi);cmi.interactions._count=(getObjLength(cmi.interactions)-2).toString();return'true';case"objectives":if(tiers[3]==="id"){count=parseInt(cmi.objectives._count,10);for(z=0;z<count;z+=1){if(cmi.objectives[z].id===v){settings.errorCode="351";settings.diagnostic="The objectives.id element must be unique.  The value '"+v+"' has already been set in objective #"+z;return'false';}}}
...
return str;}
function ISODurationToCentisec(str){var aV=[0,0,0,0,0,0],bErr=!!((str.indexOf("P")!==0)),bTFound=false,aT=["Y","M","D","H","M","S"],p=0,i=0,len;if(!bErr){str=str.substr(1);len=aT.length;for(i=0;i<len;i+=1){if(str.indexOf("T")===0){str=str.substr(1);i=Math.max(i,3);bTFound=true;}
...
function centisecsToSCORM12Duration(n){var
str,nH,nCs,nM,nS;n=Math.round(n);nH=Math.floor(n/360000);nCs=n-nH*360000;nM=Math.floor(nCs/6000);nCs=nCs-nM*6000;nS=Math.floor(nCs/100);nCs=nCs-nS*100;str="0000"+nH+":";str=str.substr(str.length-5,5);if(nM<10){str+="0";}
...
return String(result);}
this.getvalue=function(n){var v=null,lms=API.path,ec=0,m='',d='',nn=null,ig=false;if(API.isActive){switch(API.version){case"1.2":switch(n){case"cmi.comments_from_lms._count":case"cmi.comments_from_learner._count":ig=true;break;case"cmi.credit":nn="cmi.core.credit";break;case"cmi.location":nn="cmi.core.lesson_location";break;case"cmi.completion_threshold":ig=true;break;case"cmi.entry":nn="cmi.core.entry";break;case"cmi.mode":nn="cmi.core.lesson_mode";break;case"cmi.exit":nn="cmi.core.exit";break;case"cmi.score.raw":nn="cmi.core.score.raw";break;case"cmi.score.min":nn="cmi.core.score.min";break;case"cmi.score.max":nn="cmi.core.score.max";break;case"cmi.scaled_passing_score":nn="cmi.student_data.mastery_score";break;case"cmi.max_time_allowed":nn="cmi.student_data.max_time_allowed";break;case"cmi.time_limit_action":nn="cmi.student_data.time_limit_action";break;case"cmi.learner_preferences.audio_level":nn="cmi.student_preferences.audio";break;case"cmi.learner_preferences.delivery_speed":nn="cmi.student_preferences.speed";break;case"cmi.learner_preferences.language":nn="cmi.student_preferences.language";break;case"cmi.learner_preferences.audio_captioning":nn="cmi.student_preferences.text";break;case"cmi.success_status":case"cmi.completion_status":nn="cmi.core.lesson_status";break;case"cmi.session_time":nn="cmi.core.session_time";break;case"cmi.suspend_data":nn=n;break;default:nn=n;break;}
if(ig){return'false';}
v=lms.LMSGetValue(nn);break;case"2004":v=lms.GetValue(n);break;default:break;}
ec=getLastErrorCode();m=getLastErrorMessage(ec);d=getDiagnostic(ec);$(self).triggerHandler({'type':"getvalue",'n':n,'v':v,'error':{'code':ec,'message':m,'diagnostic':d}});if(ec===0||ec===403){if(v==='undefined'||v===null||v==='null'){v="";}
return String(v);}
debug(settings.prefix+": Error\nError Code: "+ec+"\nError Message: "+m+"\nDiagnostic: "+d,1);return'false';}
debug(settings.prefix+": "+n+" Get Aborted, connection not initialized! "+API.isActive,2);return'false';};this.setvalue=function(n,v){var s='false',lms=API.path,ec=0,m='',d='',nn=null,ig=false;if(API.isActive){switch(API.version){case"1.2":API.mode=API.mode===""?lms.LMSGetValue('cmi.core.lesson_mode'):API.mode;if(API.mode==="normal"){switch(n){case"cmi.location":if(v.length>255){debug(settings.prefix+": Warning, your bookmark is over the limit!!",2);}
nn="cmi.core.lesson_location";break;case"cmi.mode":nn="cmi.core.lesson_mode";break;case"cmi.exit":nn="cmi.core.exit";API.exit_type=v;break;case"cmi.score.raw":nn="cmi.core.score.raw";break;case"cmi.score.min":nn="cmi.core.score.min";break;case"cmi.score.max":nn="cmi.core.score.max";break;case"cmi.score.scaled":ig=true;break;case"cmi.success_status":case"cmi.completion_status":nn="cmi.core.lesson_status";API.data.completion_status=v;break;case"cmi.scaled_passing_score":nn="cmi.student_data.mastery_score";break;case"cmi.learner_preferences.audio_level":nn="cmi.student_preferences.audio";break;case"cmi.learner_preferences.delivery_speed":nn="cmi.student_preferences.speed";break;case"cmi.learner_preferences.language":nn="cmi.student_preferences.language";break;case"cmi.learner_preferences.audio_captioning":nn="cmi.student_preferences.text";break;case"cmi.session_time":nn="cmi.core.session_time";break;case"cmi.total_time":nn="cmi.core.total_time";break;case"cmi.suspend_data":if(v.length>4096){debug(settings.prefix+": Warning, your suspend data is over the limit!!",2);}
...
break;default:break;}
ec=getLastErrorCode();m=getLastErrorMessage(ec);d=getDiagnostic(ec);$(self).triggerHandler({'type':"setvalue",'n':n,'v':v,'error':{'code':ec,'message':m,'diagnostic':d}});if(ec===0||ec===403){return s;}
...
count=parseInt(count,10)-1;scorm.debug(settings.prefix+": Getting interaction correct responses from count "+count,4);for(i=count;i>=0;i-=1){p=self.getvalue("cmi.interactions."+n+".correct_responses."+i+".pattern");if(pattern===p){scorm.debug(settings.prefix+": Interaction Correct Responses By Pattern Returning "+i);return"match";}}
return'false';};this.init=function(){var win;try{win=window.parent;if(win&&win!==window){findAPI(window.parent);}}catch(e){debug(e,1);}
if(!API.path){try{win=window.top.opener;findAPI(win);}catch(ee){debug(ee,1);}}
if(API.path){API.connection=true;return true;}
debug(settings.prefix+": I was unable to locate an API for communication",2);if(settings.use_standalone){debug(settings.prefix+": If you included Local_API_1484_11 I'll mimic the LMS.  If not, all SCORM calls will fail.",4);settings.standalone=true;API.version="2004";API.path=typeof Local_API_1484_11==='function'?new Local_API_1484_11({cmi:settings.cmi}):null;$(API.path).on('StoreData',function(e){$(self).triggerHandler({type:'StoreData',runtimedata:e.runtimedata});});return true;}
return false;};this.getLastError=function(n){return error[n];};this.isLMSConnected=function(){return API.connection;};this.set=function(n,v){switch(n){case"version":case"createDate":case"modifiedDate":case"prefix":triggerWarning(405);return false;case"isActive":API.isActive=v;settings[n]=v;break;case"startDate":settings[n]=new Date(v);break;default:settings[n]=v;break;}
return(isError!==0);};this.get=function(n){if(settings[n]===undefined){triggerWarning(404);return false;}
return settings[n];};this.centisecsToSCORM12Duration=centisecsToSCORM12Duration;this.centisecsToISODuration=centisecsToISODuration;this.ISODurationToCentisec=ISODurationToCentisec;this.isoDateToStringUTC=isoDateToStringUTC;this.isoDateToString=isoDateToString;this.isoStringToDate=isoStringToDate;this.makeBoolean=makeBoolean;this.debug=debug;this.init();}
function SCOBot(options){"use strict";var defaults={version:"3.0.0",createDate:"04/07/2011 09:33AM",modifiedDate:"01/03/2014 12:16PM",prefix:"SCOBot",launch_data:{},interaction_mode:"state",launch_data_type:"querystring",initiate_timer:true,scorm_strict:true,scorm_edition:"3rd",success_status:"unknown",location:"",completion_status:"",suspend_data:{pages:[]},mode:"",completion_threshold:0,scaled_passing_score:0.7,max_time_allowed:'',totalInteractions:0,totalObjectives:0,startTime:0},settings=$.extend(defaults,options),lmsconnected=false,isError=false,isStarted=false,badValues='|null|undefined|false|NaN|| |',error=scorm.get('error'),self=this;function initSCO(){lmsconnected=scorm.initialize();scorm.debug(settings.prefix+": SCO Loaded from window.onload "+lmsconnected,4);if(lmsconnected){self.start();$(self).triggerHandler({'type':"load"});}
return lmsconnected;}
function exitSCO(){scorm.debug("SCO is being asked, *cough* forced to exit ...",3);if(isStarted){$(self).triggerHandler({'type':"unload"});switch(scorm.get('exit_type')){case"finish":self.finish();break;case"suspend":self.suspend();break;case"timeout":self.timeout();break;default:scorm.debug(settings.prefix+": unknown exit type",2);break;}
scorm.debug(settings.prefix+": SCO is done unloading.",4);isStarted=false;}
return true;}
...
return reg.exec(str);}
function timesUp(){scorm.debug("Times Up!");var time_action=scorm.getvalue('cmi.time_limit_action').split(','),message=!!((time_action[1]==="message"));if(message){$(self).triggerHandler({'type':"message",'text':"Time Limit Exceeded"});}
scorm.set('exit_type',"timeout");if(time_action[0]==="exit"){exitSCO();}else{$(self).triggerHandler({'type':"continue"});}}
function encodeInteractionType(type,value){var str='',str2='',i=0,arr=[],arr2=[],len,index;switch(type){case'true-false':value=value.toString();if(value==='true'||value==='false'){return value;}
scorm.debug(settings.prefix+": Developer, you're not passing true or false for true-false.  I got "+value+" instead",1);return'';case'choice':case'sequencing':if($.isArray(value)){index=0;if(value.length>36&&settings.scorm_strict){scorm.debug(settings.prefix+": Developer, you're passing a sum of values that exceeds SCORM's limit of 36 for this pattern.",2);value=value.slice(0,36);}
for(index in value){if(value.hasOwnProperty(index)){if(value[index].length>10&&settings.scorm_strict){scorm.debug(settings.prefix+": Developer, you're passing values that exceed SCORM's limit of 10 characters.  Yours have "+value[index].length+". I will truncate this as not to lose data.",2);value[index]=value[index].substring(0,10);}}}
str=value.join("[,]");value=str;}else{scorm.debug(settings.prefix+": Developer, you're not passing a array type for sequencing/choice.  I got "+typeof value+" instead\n"+JSON.stringify(value),1);value='';}
...
return value;case'matching':if($.isArray(value)){len=value.length;for(i=0;i<len;i+=1){if($.isArray(value[i])){arr.push(value[i].join("[.]"));}else{scorm.debug(settings.prefix+": Developer, you're not passing a array type for matching/performance.  I got "+typeof value+" instead",1);return'';}}
...
if($.isArray(value.answers)){len=value.answers.length;for(i=0;i<len;i+=1){if($.isArray(value.answers[i])){if($.isPlainObject(value.answers[i][1])){arr2=[trueRound(value.answers[i][1].min,7),trueRound(value.answers[i][1].max,7)];str2=arr2.join("[:]");value.answers[i][1]=str2;}
...
value=str;return value;case'numeric':if(typeof value==="number"){str=value.toString();}else if($.isPlainObject(value)){arr=[trueRound(value.min,7),trueRound(value.max,7)];str=arr.join("[:]");}else{str=parseFloat(value);if(str==="NaN"){scorm.debug(settings.prefix+": Developer, your not passing a number for a numeric interaction.  I got "+value+" instead",1);}
str+='';}
return str;case'likert':case'other':return value.toString();default:scorm.debug(settings.prefix+": Sorry, invalid interaction type detected for "+type+" on "+value,1);return false;}}
function decodeInteractionType(type,value){var i=0,arr=[],obj={},len,match=false;switch(type){case'true-false':return value;case'choice':case'sequencing':arr=value.split("[,]");value=arr;return value;case'fill-in':arr=findResponseType('case_matters',value);if(arr!==null){if(arr[0].search(/^\{case_matters=(true|false)\}$/)!==-1){obj.case_matters=arr[0].substring('{case_matters='.length,arr[0].length-1);value=value.substring(arr[0].length,value.length);scorm.debug("=== case matters"+value,4);}}
...
arr=findResponseType('lang',value);if(arr!==null){if(arr[0].search(/^\{lang=.*?\}$/)!==-1){obj.lang=arr[0].substring('{lang='.length,arr[0].length-1);value=value.substring(arr[0].length,value.length);}}
obj.text=value;return obj;case'matching':arr=value.split("[,]");len=arr.length;for(i=0;i<len;i+=1){arr[i]=arr[i].split("[.]");}
...
arr=value.split("[,]");len=arr.lengh;for(i=0;i<len;i+=1){arr[i]=arr[i].split("[.]");}
...
return'false';}
function checkProgress(){if(isStarted){var scoreRaw=0,tmpRaw=0,scoreScaled=1,progressMeasure,totalObjectivesCompleted=0,i=0,count;if(settings.totalInteractions===0||settings.totalObjectives===0){scorm.debug(settings.prefix+": Sorry, I cannot calculate Progress as the totalInteractions and or Objectives are zero",2);return'false';}
...
progressMeasure=(totalObjectivesCompleted/settings.totalObjectives).toString();scorm.setvalue('cmi.progress_measure',trueRound(progressMeasure,7));settings.completion_status=(parseFloat(progressMeasure)>=parseFloat(settings.completion_threshold))?'completed':'incomplete';scorm.setvalue('cmi.completion_status',settings.completion_status);settings.success_status=(parseFloat(scoreScaled)>=parseFloat(settings.scaled_passing_score))?'passed':'failed';scorm.setvalue('cmi.success_status',settings.success_status);return{score_scaled:scorm.getvalue('cmi.score.scaled'),success_status:scorm.getvalue('cmi.success_status'),progress_measure:scorm.getvalue('cmi.progress_measure'),completion_status:scorm.getvalue('cmi.completion_status')};}
...
settings.completion_status=scorm.getvalue('cmi.completion_status');settings.success_status=scorm.getvalue('cmi.success_status');settings.comments_from_lms=getCommentsFromLMS();if(settings.comments_from_lms!=='false'){$(self).triggerHandler({'type':"comments_lms",'data':settings.comments_from_lms});}
...
return'true';}
return notStartedYet();};this.startTimer=function(){var time=scorm.ISODurationToCentisec(settings.max_time_allowed)*10;setTimeout(timesUp,time);};this.debug=scorm.debug;this.getvalue=scorm.getvalue;this.setvalue=scorm.setvalue;this.getMode=function(){if(isStarted){return settings.mode;}
...
return notStartedYet();};this.progress=checkProgress;this.setSuspendDataByPageID=function(id,title,data){if(isStarted){var i,len=settings.suspend_data.pages.length;for(i=0;i<len;i+=1){if(settings.suspend_data.pages[i].id===id){settings.suspend_data.pages[i].data=data;scorm.debug(settings.prefix+": Suspend Data Set",4);scorm.debug(settings.suspend_data,4);return setSuspendData();}}
...
return notStartedYet();};this.getSuspendDataByPageID=function(id){if(isStarted){var i,len=settings.suspend_data.pages.length;for(i=0;i<len;i+=1){if(settings.suspend_data.pages[i].id===id){return settings.suspend_data.pages[i].data;}}
return'false';}
return notStartedYet();};this.getSecondsFromStart=function(){return settings.startTime-currentTime();};this.setInteraction=function(data){if(isStarted){var n,m,i,j,p,p1='cmi.interactions.',p2,orig_timestamp=data.timestamp||scorm.isoStringToDate(scorm.getvalue(p1+scorm.getInteractionByID(data.id)+'.timestamp')),timestamp,latency,result,len,key;if(!$.isPlainObject(data)){scorm.debug(settings.prefix+": Developer, your not passing a {object} argument!!  Got "+typeof data+" instead.",1);return'false';}
...
p2='correct_responses._count';if($.isArray(data.correct_responses)){len=data.correct_responses.length;for(j=0;j<len;j+=1){p=scorm.getInteractionCorrectResponsesByPattern(n,data.correct_responses[j].pattern);scorm.debug(settings.prefix+": Trying to locate pattern "+data.correct_responses[j].pattern+" resulted in "+p,4);if(p==='false'){p=scorm.getvalue(p1+p2)==='-1'?0:scorm.getvalue(p1+p2);scorm.debug(settings.prefix+": p is now "+p,4);}
if(p==="match"){scorm.debug(settings.prefix+": Developer, I've already added this correct response type '"+data.correct_responses[j].pattern+"'",2);}else{result=scorm.setvalue(p1+'correct_responses.'+p+'.pattern',encodeInteractionType(data.type,data.correct_responses[j].pattern));}}}else{scorm.debug(settings.prefix+": Something went wrong with Correct Responses, it wasn't an Array.",1);}
...
return result;}
return notStartedYet();};this.getInteraction=function(id){if(isStarted){var n,p1='cmi.interactions.',m,p,i=0,obj={},ts,ly,timestamp,latency;n=scorm.getInteractionByID(id);if(n==='false'){return n;}
p1+=n+'.';obj.id=id;obj.type=scorm.getvalue(p1+'type');m=scorm.getvalue(p1+'objectives._count');ts=scorm.getvalue(p1+'timestamp');ly=scorm.getvalue(p1+'latency');timestamp=(isISO8601(ts))?scorm.isoStringToDate(ts):ts;latency=(isISO8601(ly))?scorm.isoStringToDate(ly):ly;obj.objectives=[];if(m!=='false'){for(i=0;i<m;i+=1){obj.objectives.push({id:scorm.getvalue(p1+'objectives.'+i+'.id')});}}
obj.timestamp=timestamp;p=scorm.getvalue(p1+'correct_responses._count');obj.correct_responses=[];if(p!=='false'){for(i=0;i<p;i+=1){obj.correct_responses.push({pattern:decodeInteractionType(obj.type,scorm.getvalue(p1+'correct_responses.'+i+'.pattern'))});}}
...
return notStartedYet();};this.setObjective=function(data){if(isStarted){var p1='cmi.objectives.',n=scorm.getObjectiveByID(data.id),result='false',f=false,def1=": Passed no or bad ",def2=" ignored.",sv=scorm.setvalue,key;scorm.debug(settings.prefix+": Setting Objective at "+n+" (This may be false)");if(isBadValue(n)){n=scorm.getvalue(p1+'_count');if(n==='false'){scorm.debug(settings.prefix+": LMS is return false, can not proceed, check error codes");return n;}
scorm.debug(settings.prefix+": Objective "+data.id+" was not found.  Adding new at "+n+" "+data.description);f=true;}
p1+=n+'.';if(f){if(!isBadValue(data.id)){sv(p1+'id',data.id.toString());}else{scorm.debug(settings.prefix+": You did not pass an objective ID!!  What I did get below:",1);for(key in data){if(data.hasOwnProperty(key)){scorm.debug("key: "+key+"\n value: "+data[key]);}}
return'false';}}
if($.isPlainObject(data.score)){result=!isBadValue(data.score.scaled)?sv(p1+'score.scaled',trueRound(data.score.scaled,7).toString()):scorm.debug(settings.prefix+def1+p1+"score.scaled: "+data.score.scaled+def2,3);result=!isBadValue(data.score.raw)?sv(p1+'score.raw',trueRound(data.score.raw,7).toString()):scorm.debug(settings.prefix+def1+p1+"score.raw: "+data.score.raw+def2,3);result=!isBadValue(data.score.min)?sv(p1+'score.min',trueRound(data.score.min,7).toString()):scorm.debug(settings.prefix+def1+p1+"score.min: "+data.score.min+def2,3);result=!isBadValue(data.score.max)?sv(p1+'score.max',trueRound(data.score.max,7).toString()):scorm.debug(settings.prefix+def1+p1+"score.max: "+data.score.max+def2,3);}else{scorm.debug(settings.prefix+": Did not receive a score object.  May or may not be an issue.",4);}
result=!isBadValue(data.success_status)?sv(p1+'success_status',data.success_status):scorm.debug(settings.prefix+def1+p1+"success_status: "+data.success_status+def2,3);result=!isBadValue(data.completion_status)?sv(p1+'completion_status',data.completion_status):scorm.debug(settings.prefix+def1+p1+"completion_status: "+data.completion_status+def2,3);result=!isBadValue(data.progress_measure)?sv(p1+'progress_measure',data.progress_measure):scorm.debug(settings.prefix+def1+p1+"progress_measure: "+data.progress_measure+def2,3);result=!isBadValue(data.description)?sv(p1+'description',data.description):scorm.debug(settings.prefix+def1+p1+"description: "+data.description+def2,3);scorm.debug(settings.prefix+": Progress\n"+JSON.stringify(checkProgress(),null," "),4);return result.toString();}
...
return notStartedYet();};this.setCommentFromLearner=function(msg,loc,date){if(isStarted){var p1="cmi.comments_from_learner.",n=scorm.getvalue(p1+"_count");if(n==='false'){scorm.debug(settings.prefix+": Sorry, LMS returned a comments count of 'false'.  Review error logs.");return n;}
...
settings.completion_status=(parseFloat(progressMeasure)>=parseFloat(settings.completion_threshold))?'completed':'incomplete';scorm.setvalue('cmi.completion_status',settings.completion_status);settings.success_status=(parseFloat(scoreScaled)>=parseFloat(settings.scaled_passing_score))?'passed':'failed';scorm.setvalue('cmi.success_status',settings.success_status);return'true';};this.happyEnding=function(){if(isStarted){scorm.setvalue('cmi.score.scaled','1');scorm.setvalue('cmi.score.min','0');scorm.setvalue('cmi.score.max','1');scorm.setvalue('cmi.score.raw','1');scorm.setvalue('cmi.success_status','passed');scorm.setvalue('cmi.progress_measure','1');return scorm.setvalue('cmi.completion_status','completed');}
return notStartedYet();};this.commit=function(){if(isStarted){return scorm.commit('');}
return notStartedYet();};this.suspend=function(){if(isStarted){scorm.debug(settings.prefix+": I am suspending...",3);scorm.setvalue('cmi.exit','suspend');updateStatus();isStarted=false;return scorm.terminate();}
return notStartedYet();};this.finish=function(){if(isStarted){scorm.debug(settings.prefix+": I am finishing...",3);scorm.setvalue('cmi.exit','normal');updateStatus();isStarted=false;return scorm.terminate();}
return notStartedYet();};this.timeout=function(){if(isStarted){scorm.debug(settings.prefix+": I am timing out...",3);scorm.setvalue('cmi.exit','time-out');updateStatus();isStarted=false;return scorm.terminate();}
