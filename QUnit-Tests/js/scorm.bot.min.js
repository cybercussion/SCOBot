...
function adlGetValue(key){console.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");var r="false";if(key.indexOf('adl.nav.request_valid.choice')>=0){settings.errorCode=301;scorm.debug(settings.prefix+"Sorry, targeted 'choice' requests not allowed by this API since there is no navigation.",2);}else{console.log(adl);r=getData(key.substr(4,key.length),adl);if(r==='undefined'){settings.errorCode=401;settings.diagnostic="Sorry, there was a undefined response from "+key;r="false";}}
scorm.debug(settings.prefix+": GetValue "+key+" = "+r,4);return r;}
...
if(settings.adl!==null){adl=settings.adl;}else{console.log("SETTING ADL TO ADL");adl=settings.ADL;}
